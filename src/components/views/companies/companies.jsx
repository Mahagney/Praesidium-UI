...
function Companies({ companies, addCompany, deleteCompany, updateCompany }) {
  ...
  const validate = {
    [constants.NAME]: (s) => !!(!s || s.length < 3),
    [constants.EMAIL]: (s) => !!(!s || !EmailValidator.validate(s)),
    [constants.CUI]: (s) => !!(!s || s.length < 8),
    [constants.PHONE_NUMBER]: (number) => {
      let phoneNumber = null;
      if (number) phoneNumber = parsePhoneNumberFromString(number, 'RO');
    ...
    [constants.DOMAIN]: (s) => !!(!s || s.length < 2),
  };
  ...
  const actionFunction = (callback, newData) =>
    new Promise((resolve, reject) => {
      setTimeout(() => {
        if (
          validate[constants.NAME](newData[constants.NAME]) ||
