...
function PdfViewer({ onDocumentLoadSuccess, goToPrevPage, goToNextPage, pageNumber, numPages, url, width }) {
  const classes = useStylesPdfViewer();
  let windowWidth = Math.round(width * 0.9);
  if (windowWidth && windowWidth > 800) windowWidth = 1000;

  return (
    <div className={classes.pdfContainer}>
      <nav>
        <Button variant="contained" color="primary" onClick={goToPrevPage} disabled={pageNumber == 1 || !numPages}>
      ...
      <Document file={url} onLoadSuccess={onDocumentLoadSuccess} className={classes.pdfFileContainer}>
